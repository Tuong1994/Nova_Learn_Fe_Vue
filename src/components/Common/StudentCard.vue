...
defineComponent({ name: "NVLStudentCard" });

interface NVLStudentCardProps {
  langs: Langs;
  student: IStudentCard;
}

defineProps<NVLStudentCardProps>();
</script>

<template>
  <Card isHover wrapClass="nvl-student-card" bodyClass="nvl-student-card-body">
    <template #body>
      <Row align="start" justify="spaceBetween">
        <Col :xs="24" :span="10">
          <div class="card-image">
            <div class="image-wrap">
              <Image fit="cover" :src="student.imageUrl" />
            </div>
          </div>
        </Col>
        <Col :xs="24" :span="13">
          <Title wrapClass="card-title" theme="light" :level="6">
            {{ student.name }}
          </Title>
          <InfoRow
            theme="light"
